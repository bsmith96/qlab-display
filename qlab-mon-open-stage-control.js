...
// @version 0.1.0
// @about Monitoring the next cue in a specified cue list using Open Stage Control
...
const openStageControlIP = "127.0.0.1";
const openStageControlPort = 7000;
...
qlabReplies.on("message", function(oscMessage) {
  ...
  if(oscMessage.address == "/reply/settings/audio/outputChannelNames") { // get names of output channels
    ...
    if (cueNumber !== "none") {
      ...
      sendToOpenStageControl(openStageControlQName, "no click standing by");
      console.clear();
      console.log("No cue");
    }
  };
